...
train_path_pattern=/nobackup/scratch/Wed/francl/toneRecords_fullrange_jitteredPhase_man-added-ILD_stackedCH_upsampled/train*.tfrecords
model_version=50000,100000
...
SINGULARITYENV_LD_PRELOAD=/usr/lib/libtcmalloc.so.4 singularity exec --nv -B /om -B /nobackup -B /om2 tfv1.13_tcmalloc.simg python -u call_model_testing_valid_pad_francl_training_no_noise_anechoic_psychophysics.py $trainingID $init "$reg" "$bkgd" "$pattern" "$model"
