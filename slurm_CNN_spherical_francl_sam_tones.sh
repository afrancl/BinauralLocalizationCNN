...
train_path_pattern=/nobackup/scratch/Wed/francl/samHarmonicToneRecords_fulllengthTime_jitteredCarrier_jitterSlice_stackedCH_48kHzsynthesized/train*.tfrecords
...
SINGULARITYENV_LD_PRELOAD=/usr/lib/libtcmalloc.so.4 singularity exec --nv -B /om -B /nobackup -B /om2 tfv1.13_tcmalloc.simg python -u call_model_testing_valid_pad_sam_tones.py $trainingID $init "$reg" "$bkgd" "$pattern" "$model"
