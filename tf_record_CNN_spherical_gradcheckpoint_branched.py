...
import json
import pdb
from NetBuilder_2GPUbranch import NetBuilder
    ...
    if testing:
        ...
        try:
            ...
            if tone_version:
                ...
                np.save(newpath+'/plot_array_{}_iter{}.npy'.format(stimuli_name, model_version),batch_acc)
                np.save(newpath+'/batch_conditional_{}_iter{}.npy'.format(stimuli_name, model_version),batch_conditional)
                acc_corr=[pred[0] for pred in batch_acc]
                acc_accuracy=sum(acc_corr)/len(acc_corr)
                if branched:
                    np.save(newpath+'/plot_array_{}_2_iter{}.npy'.format(stimuli_name, model_version),batch_acc2)
                    np.save(newpath+'/batch_conditional_{}_2_iter{}.npy'.format(stimuli_name, model_version),batch_conditional2)
                    acc_corr2=[pred2[0] for pred2 in batch_acc2]
                    acc_accuracy2=sum(acc_corr2)/len(acc_corr2)
