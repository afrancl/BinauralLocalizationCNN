...
import time
import json
sys.path.insert(0,'/om/user/francl/py-cochleagram/')
    ...
    with tf.variable_scope('fp32_storage', custom_getter=float32_variable_storage_getter):
       ...
       # print(subbands_batch)
        ...
        if branched:
            ...
            cost2 = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(logits=out,labels=class_num_batch))
            cost = cost1 +cost2
        else:
...
#     print(sess.run(cost))
    ...
    grads = gradients_with_loss_scaling(cost,loss_scale)
    grads_checked = [(tf.check_numerics(grad[0],"NAN/Inf found in gradients!"),
                     grad[1]) for grad in grads]
    update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS)
    with tf.control_dependencies(update_ops):
        update_grads = tf.train.AdamOptimizer(learning_rate=learning_rate,epsilon=1e-4).apply_gradients(grads_checked)
    ...
    if testing:
        ...
        try:
            ...
            if tone_version:
                ...
                np.save(newpath+'/plot_array_padded_{}.npy'.format(stimuli_name),batch_acc)
                np.save(newpath+'/batch_conditional_{}.npy'.format(stimuli_name),batch_conditional)
                acc_corr=[pred[0] for pred in batch_acc]
                acc_accuracy=sum(acc_corr)/len(acc_corr)
                if branched:
