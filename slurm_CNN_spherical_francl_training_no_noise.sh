...
SINGULARITYENV_LD_PRELOAD=/usr/lib/libtcmalloc.so.4 singularity exec --nv -B /om -B /nobackup -B /om2 tfv1.13_tcmalloc.simg python -u call_model_testing_valid_pad_francl_training_no_noise.py $trainingID $init "$reg" "$bkgd" "$pattern" "$model"
